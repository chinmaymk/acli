...
import type { CommandInfo } from './cmd/commands.js';
  ...
  if (rawArgs.length === 1 && rawArgs[0] === '-') {
    return new Promise((resolve, reject) => {
      ...
      process.stdin.on('end', () => {
        ...
        let args: string[];
        try {
          args = JSON.parse(trimmed) as string[];
        } catch (err) {
          reject(new Error(`invalid stdin argument JSON array: ${(err as Error).message}`));
          return;
        }
        resolve(args);
      });
