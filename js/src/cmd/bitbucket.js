...
const paginationOptions = {
  ...
  pagelen: { type: 'number', description: 'Page length', default: 0 },
  all: { type: 'boolean', description: 'Fetch all pages', default: false },
};
 ...
 */
function resolveWorkspaceRepoIDAndTaskID(argv, args) {
  if (args.length >= 4) {
 ...
 */
export function registerBitbucketCommands(yargs) {
  return yargs
        // ─── whoami ──────────────────────────────────────────────────────
        .command(
          'whoami',
          ...
          (wsCmd) => {
            wsCmd
              ...
              .command(
                ...
                (y) => y.positional('workspace', { type: 'string' }),
                async (argv) => {
                  const args = argv.workspace ? [argv.workspace] : [];
                  const workspace = defaultWorkspace(argv, args, 0);
                  const client = getBitbucketClient(argv);
            ...
            envCmd
              ...
              .command(
                ...
                async (argv) => {
                  ...
                  const rankMap = { Production: 2, Staging: 1 };
